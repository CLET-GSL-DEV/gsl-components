...
- **"Clet"** introduced as the library/design-system's own brand name (distinct from the "GSL" institutional abbreviation, which stands for Ghana School of Law and is unchanged) — added to the docs site header, README, AGENTS.md, and the component-authoring cursor rule title. No functional identifiers changed: the `@rfdtech/components` package name, `gsl-*` CSS class names, and the MCP server id (`gsl-components-docs`) are all unchanged.
