...
- **AccessibleName**: new `src/types/accessible-name.ts` union — `Dropdown`, `Combobox`, `Dialog`, `Modal`, `UploadField` and `AppHeaderSearch` must carry exactly one of `aria-label`/`aria-labelledby`, and carrying neither is a type error. A control with a visible `<label>` points at it with `aria-labelledby` instead of growing a second invisible name. The `migrate` codemod reports unnamed pickers with file and line.
...
- **Sidebar**: new `variant="brand"`, the 2.4 rail — deep-navy surface with the Adinkra artwork docked right at full height, an image-only header (`SidebarBrand` renders the bundled CLET mark with no props; explicit `logo`/`title` still override), uppercase `#76808d` group labels, 36 px items and a white active pill with navy text. `variant="primary"` is frozen at the 2.3 look and `versions.css` keeps it there for pinned shells.
- **HeroBanner**: edge-to-edge dashboard hero — greeting (default "Good morning,"), name, optional role, the current date (overridable), and preset artwork selected by index. All copy is props; only the date defaults and the images are presets. Built for the main dashboard page, placed under the page/section title.
- **Banner**: the page-level persistent notice, replacing `Notice` (now deprecated). `info`/`success`/`warning`/`danger` variants carry the colour through a 4 px accent bar, the background tint and the action link together, `heading`/`subtext` take the copy, `action` is the consumer's slot, and the dismiss button renders only when `onClose` is passed. `danger` renders `role="alert"`.
- **EmptyState**: illustrated empty state for tables and lists — artwork (the bundled CLET illustration by default), configurable `title`/`description`, and an optional `action` slot. `TableContent` renders it by default when `data` is empty; `emptyText` feeds the title, `emptyIcon` is the legacy path, and `emptyContent` takes a fully custom state. Headers keep rendering above it.
- **AppHeaderSearch**: new `collapsible` mode — the field renders as a round trigger that swaps itself for the input on click (autofocused); Escape or the close button collapses and clears. Controlled (`collapsed`/`onCollapsedChange`) or uncontrolled (`defaultCollapsed`).
- **AppHeaderTitle**: the system title for the 2.4 header. The rail is image-only now, so the app's name renders here; the component carries the size, weight and spacing — drop it in, no styling needed.
- **MetricCard**: new `descriptionAdornment` prop — any node rendered left of the footer description (the dashboard set uses small coloured icons there).
- **Version pins**: `AppLayout` takes `version`, `VersionPin` wraps any subtree, and `src/styles/versions.css` freezes older releases additively — latest is the default, a pin restores a frozen look without forking markup. Exports: `CletVersion`, `LATEST_CLET_VERSION`, `isCletVersion`.
- **Theme**: Matimo is the unified 2.4 typeface — all four weights ship embedded in the published CSS, `--clet-font*` lead with it, and the 2.3 Lato/Roboto Serif stacks are frozen under the 2.3 pin. New `#0c4a6e` accents (hero scrim, Banner info, avatar fill) and the profile avatar now renders solid navy with a gold border.

### Changed

- **Table**: `variant="soft"` is now the default on `Table` and `TableContent` — tinted header band, hairline dividers, pill controls. The bordered grid survives as the explicit `variant="default"` (legacy, kept for 2.3 pins).
- **Notice** is deprecated in favour of `Banner`; it still renders and is frozen for 2.3 pins.
- **CLI**: `rfdui migrate` now drives the 2.4 cut — `Sidebar primary→brand`, `SidebarBrand` collapses to the baked-in mark with the rail title promoted into an `AppHeaderTitle`, `AppHeaderSearch` moves into the actions row as a collapsible trigger, `Notice→Banner` with variant mapping and `children→subtext`, `MetricCard` `icon` moves into a coloured `descriptionAdornment`, `emptyIcon` retires in favour of the default empty state, and hero-banner candidates are reported for the AI to triage down to the main dashboard.

### Deprecated

- **Notice**: use `Banner`. See the Banner page for the mapping (`default/info→info`, `error→danger`, `leftBorder`/`dashed`/`icon`/`color` have no equivalent).

### Added

- **Theme**: the accessibility text size now lives on `ThemeProvider`, beside the colour mode, so an app that already has the provider gets it with nothing extra to mount. Four steps, `sm`, `md`, `lg` and `xl`, where `md` is the shipped default and reproduces today's sizes exactly. `useTheme()` returns `fontSize` and `setFontSize`, and `useFontSize()` is the narrower hook for a control that only needs those two. The step persists under `clet-font-size` and takes `fontSize`/`onFontSizeChange` for controlled use, matching how the colour mode already works.
- **Theme**: new `--clet-text-scale` global token, the multiplier every `--clet-text-size-*` step is now defined against. `ThemeProvider` writes `data-clet-font-size` to the `documentElement` rather than to its own wrapper element, which is what lets portaled surfaces (dropdown and combobox menus, dialogs, sheets, tooltips, the command palette) scale with the page instead of staying behind at the default. It is deliberately not mapped into Tailwind: it is a multiplier rather than a length, and it is already baked into the `--text-*` utilities through the size tokens.
- **UploadField**: file names are now checked before a file is accepted, and a selection containing an unsafe name is refused whole rather than partly. Four cases are caught: a NUL or other C0 control character (the poison-null-byte trick, where `shell.php\0.jpg` truncates in a C string API so the extension a server validates is not the one it writes), a path separator that lets a name escape its directory, a bidirectional override that reorders the rendered text so the real extension reads as something harmless, and an empty name. The rejected name is echoed back with the offending characters replaced by `U+FFFD`, since printing it verbatim would let a bidi override disguise it inside the error dialog too. The rule is exported as `findUnsafeFileNameReason` and `isSafeFileName` so an app can run it before calling its own API. This is defence in depth, not a security boundary: anything in the browser is bypassed by posting to the API directly, so the server has to run the same check.
- **UploadField**: new `subtitle` prop, so the hint under the title can be written by the consumer instead of only derived. Pass `""` to drop the line entirely. The derived text remains the default.
- **AppHeaderFontSize**: new component, an "Aa" trigger that opens the text-size picker, sized for `AppHeaderActions` and conventionally placed just before `AppHeaderNotifications`. Options are exposed as a radio group with the active step marked, and each one previews its own size. It reads the size from `ThemeProvider` and renders nothing when there is no provider above it, rather than throwing.
- **Layout shell**: the brand colour moved from the top bar onto a full-height rail. `AppLayout`'s default arrangement is now flush, the sidebar runs the full height against the viewport edge and the header spans the top of the content column only, with nothing inset or rounded, and it pairs with a new `Sidebar variant="primary"` and a redefined `AppHeader variant="plain"`. Adopt the three together.
- **Sidebar**: new `variant="primary"`, the brand rail. `--clet-primary-surface` background, on-primary text, flush edges and no border, a translucent hover and active pill with an inset outline, uppercase group labels, and a fold chevron that points up while a group is open. Every value repaints one of the existing `--clet-sidebar-*` override points rather than introducing a parallel set, so `cletTheme({ components: { sidebar: … } })` re-themes the rail through the documented token names. The foreground scale derives from `--clet-on-primary` at decreasing opacities, and the focus ring is forced to `--clet-on-primary` because `--clet-focus` resolves to the rail's own background.
- **AppHeader**: new `variant="primary"`, square edges, brand background, on-primary text. This is what `variant="plain"` used to be; see Changed below.
- **AppLayout**: new `variant="panel"`, the pre-2.3 default, where the sidebar, header and content float as inset rounded panels. `variant="stacked"` is unchanged.
- **Theme**: new `--clet-primary-surface` global token (plus the `--color-primary-surface` Tailwind mapping) for brand-coloured chrome, the primary rail and the primary header. It equals `--clet-primary` in light and deepens in dark, where `--clet-primary` lightens for contrast against dark text surfaces and would otherwise make a full-height rail the brightest thing on screen.
- **MetricCard**: new `variant="soft"`, the current preferred card. Borderless on a `--clet-shadow-md` lift, carrying the same `--clet-radius-xl` corner the `Card` component uses so a row of metrics and a card below them share one radius, with a brand watermark bleeding off the bottom-right corner and clipped by the card's own overflow. Five marks ship with it (`rings`, `lattice`, `quatrefoil`, `bird`, `crossed`) at ~17 kB each. `mark` takes an id, a node of your own, or `false`; omit it and one is chosen by hashing `label` with FNV-1a, so the same label always draws the same mark and a row of differently-labelled cards spreads across the set without anyone picking. Both of the mark's offsets derive from `--clet-metric-card-mark-size` at a fixed 3:1 right-to-bottom ratio, so scaling it keeps the crop identical. The title also drops the eyebrow treatment here and reads as a card heading.
- **Table**: new `variant="soft"` on the root, and a matching `TableContent variant="soft"`. The header puts filters on the left and inputs on the right, rounds the filter trigger, search field and dropdowns into pills on a faint shadow, and the pagination draws the current page as a filled `--clet-info` disc with Previous/Next collapsed to their chevrons. All of it reaches those controls by drilling down from the root class rather than swapping in different components, so `Dropdown`, `TableSearch` and `TablePagination` keep their own APIs, URL state and keyboard behaviour. `TableContent variant="soft"` drops the outer border for a tinted header band and hairline row rules.
- **Theme**: new `--clet-shadow-soft` global token (`0 0 10px rgb(0 0 0 / 10%)`, plus a `--shadow-soft` Tailwind mapping), an even, directionless lift shared by every surface in the soft design language, so a row of metric cards and the filter pills below them sit on exactly the same shadow rather than two that nearly match.
- **Theme**: new `--clet-info` / `--clet-on-info` global tokens (plus `--color-info` / `--color-info-foreground` Tailwind mappings) for the "you are here" blue, deliberately not `--clet-primary`, since the brand navy reads as chrome where a selected page needs to read as a live state.
- **AppHeaderNotifications**: the popover is now split by pill tabs, **New** for unread, **History** for the rest, partitioned from the `unread` prop on each `AppHeaderNotificationItem`, with an empty state per tab. A popover whose children aren't `AppHeaderNotificationItem`s can't be partitioned, so it renders as one untabbed list exactly as before.
- **SidebarFooter**: `children` is now optional. With none, the footer renders the CLET wordmark, so the rail always has a bottom anchor instead of trailing off into empty space; any child replaces it. Sized by `--clet-sidebar-wordmark-size` and hidden when the rail is collapsed.
- **CLI**: new `rfdui migrate` codemod for the shell change. Rewrites `AppLayout`/`AppHeader`/`Sidebar` variants across an app, either adopting the new shell (default) or pinning the old appearance (`--preserve`); a dry run until `--write`. It parses JSX with the TypeScript compiler resolved from the app being migrated, so multi-line props, aliased imports and same-named local components are all handled correctly, and anything it cannot decide safely, a computed `variant`, a namespace import, a bare `<Sidebar>` whose layout may not have gone flush, is reported with a file and line instead of guessed at. It also carries the mechanical half of the v1 to v2 migration: `AppHeaderProfile` becomes `ProfilePopover` (import, both JSX tags, the `onProfileClick`/`onSettingsClick`/`onHelpClick` callbacks, and the dead `headerAction` prop), and `gslTheme` plus every `Gsl*` type alias become their `Clet*` equivalents. It refuses to rename into an existing binding, and leaves `.gsl-*` class names alone, they are a permanent alias, and a blind prefix swap would also rename a consumer's own `gsl-`-prefixed classes. Everything needing a judgement call, an `AppSwitcher` grid to re-author, a `selectable` table's dropped kebab column, brand token overrides to keep or drop, a deprecated `CountrySelector`/`NetworkOperator`, is reported rather than rewritten, and the migration guide is now written around running it.
- **AppLayout**: new `hideHeader` and `hideSidebar` booleans. Either one drops that region whatever children were passed: the wrapper element is not emitted, no column or row is reserved, and the content takes the space. The children are untouched, so the same tree renders whole when the flags are off. Written for federated apps, where a remote renders the same layout with the flags on and the host draws the chrome once. `SidebarProvider` and `BreadcrumbProvider` stay mounted either way, so `useSidebar()` and `useBreadcrumbs()` keep working in a remote whose sidebar the host owns.
- **TableFilter**: filter fields are now checked in development. Every field that takes a value has to declare `name` (the filter key), and every `f_*` param in the URL has to be carried by a field rendering that same value. Each violation is reported to the console naming the field and the fix. A field without a `name` keeps its value to itself, never reaches the table's state and is left behind by "clear", which until now failed silently.
- **Hooks**: new `useTableFilterReset` and `TABLE_FILTER_RESET_EVENT` exports, the signal a `TableFilter` dispatches when "clear" is pressed. The kit's own filter fields answer it; a custom field subscribes with the hook.

### Changed

- **AppLayout**: `variant="default"` changed meaning. It used to be the inset rounded-panel arrangement, which is now `variant="panel"`; the default is the flush shell. Every `<AppLayout>` without a variant takes the new look on upgrade, `rfdui migrate --preserve` pins them all back to `"panel"`.
- **AppHeader**: `variant="plain"` changed meaning. It used to be the brand-coloured bar, which is now `variant="primary"`; `"plain"` is the page surface with ordinary text and one hairline along the bottom. Existing plain headers go from coloured to quiet on upgrade, which is the point of the change, `rfdui migrate --preserve` moves them to `"primary"` instead. The two hardcoded `rgb(255 255 255 / 15%)` hover washes moved with it and are now `color-mix` on `--clet-on-primary`, exposed as `--clet-app-header-hover-bg`.
- **AppHeader**: the brand-coloured bar (now `variant="primary"`) paints `--clet-primary-surface` rather than `--clet-primary`, so it matches the rail and stops rendering as a bright band in dark mode. Light mode is unchanged.
- **AppLayout**: the sidebar column now stacks one level above the header rather than at the same level. At equal z-index the header won on DOM order, and an opened mobile drawer slid out underneath it.
- **Sidebar**: `mobileHeader` auto-injection now applies to `variant="primary"` as well as `variant="plain"`. A sidebar that supplies its own `SidebarHeader`, the usual composition on the rail, still opts out, as before.
- **Docs**: the page-composition rules describe the new shell. Branding moved from `AppHeaderBranding` to a `SidebarHeader` on the rail; `SidebarFooter` is still never omitted, but the identity block now belongs in exactly one place, a `variant="full"` header popover with a wordmark in the footer, or a footer popover with a `variant="avatar"` header, rather than always in the footer.
- **TableFilter**: `variant="spread"` is now honoured for up to two fields. A filter holding a third groups itself back into the popover behind one trigger, whatever the markup asks for, and says so once in the dev console. Three controls laid out inline stop reading as a filter row and start competing with the search field beside them; on a narrow viewport they wrap into a stack of dropdowns above the data. The fields, their names, their URL keys and "clear" are identical in both variants, so only the presentation moves. `page-composition-table-filter-count` is the matching rule for new work.
- **TableFilter**: "clear" now empties every named field, including controlled ones a native form reset cannot reach, instead of restoring the value a `Dropdown` happened to mount with. It also deletes only the filter params the form owns, so a param belonging to something else on the page survives; it used to drop every `f_*` key and rewrite them from the form, destroying any it had no field for.
- **TableFilter**: the `spread` row stays inline with its content at desktop widths and stacks only in the mobile block, rather than wrapping fields onto their own lines. The "clear" action renders only when a filter is actually applied.
- **BulkImportModal**: spreadsheet parsing no longer uses the `xlsx` package, whose npm release is frozen at 0.18.5 with an unpatched CVE. `.csv` files parse with `papaparse` and `.xlsx` with `read-excel-file`, both optional peers to install alongside the library. **BREAKING**: legacy `.xls` files are refused with a re-save-as-`.xlsx` hint instead of parsed, since the new reader only supports the modern format.
- **ExportButton**: `exportToXlsx` now writes through the `write-excel-file` optional peer instead of `xlsx`, and is async, so `await` it when calling it directly rather than through `ExportButton`.
- **Toolchain**: major bumps for the dev/build line — `vite` 6 to 8, `typescript` 5.9 to 6.0, `vite-plugin-dts` 4 to 5, `jsdom` 29 to 30, `@testing-library/jest-dom` 6 to 7. Three compat fixes came with them: the build script no longer assumes the `next` CSS asset name, `tsconfig.build.json` sets `rootDir` explicitly, and `*.css` side-effect imports have a shorthand ambient declaration. `@types/mdx` is now an explicit dev dependency (two docs files import `mdx/types` from it; it previously resolved only by accident).
- **AppHeaderSearch**: `collapsible` now collapses back to the icon button when focus leaves the field — expanding still autofocuses the input, but the field no longer stays open after you click away. Escape and the close button behave as before.

### Removed

- **BREAKING. LogoLoader**: removed at 2.3, along with its docs page, its bundled mark and every `--clet-logo-loader-*` token. Anything importing `LogoLoader` will not compile against this release.
- **BREAKING. Card**: `loading`, `loadingLabel` and `loadingMinHeight` are removed with it, so a card no longer swaps its children for a loader. Use the loading state of the component inside it (`TableContent`, `MetricCard`, `Launchpad`, `ProfilePopover`), or supply your own.

### Fixed

- **Theme**: the unified typeface is applied, not just declared. The `--clet-font*` tokens named Matimo but nothing put a `font-family` on the theme wrapper, so every component had to declare its own and any text outside those declarations inherited the host app's body font. The wrapper applies `--clet-font-body` now, and a zero-specificity reset makes `button`, `input`, `select` and `textarea` inherit it: form controls never inherit from the cascade, which is how the Launchpad's tile buttons rendered in the browser default while the modal title beside them rendered Matimo. The reset is unscoped because Radix portals mount to `document.body`, outside every wrapper, and the portal roots already declare the family for their contents to inherit.
- **Version pins**: the frozen 2.3 type stacks actually freeze now. `base.css` re-declared the Matimo tokens on `.clet-theme` itself, and a direct declaration on the wrapper beats anything a `VersionPin` ancestor passes down by inheritance, so a pinned shell kept rendering Matimo no matter what `versions.css` said. The font tokens live on `:root` alone, the same place `--clet-text-scale` already occupied for exactly this reason, and themed subtrees inherit them: a `data-clet-version="2.3"` wrapper now resolves Lato body and Roboto Serif headers inside, and an unpinned app is unchanged. The Matimo webfont itself was already embedded (all four faces ship as data URIs inside the published `dist/index.css` and `dist/next.css`), so no consuming-app font setup was ever needed; the code comment claiming otherwise is corrected.
- **Every component**: text sizes are tokenised. 150 `font-size` declarations across 36 component stylesheets hardcoded a pixel literal, and 7 more sat behind component tokens that terminated in one, so only about a quarter of the library's text answered to the size tokens at all. Values already on the global scale now read the matching `--clet-text-size-*` token; the off-scale ones (10, 11, 15, 18, 28, 32 and 40px) each gained a scale-aware component token declared on `:root` in that component's own stylesheet, so it inherits into portaled parts and a `cletTheme()` override at the component selector still wins. Every value is identical at the default step, so nothing moves until someone changes the setting. `Avatar` also stopped setting `fontSize` inline for its named sizes, which had been silently overriding the stylesheet's own `--sm`/`--md`/`--lg` rules and making them dead code.
- **Controls**: a control that carries text no longer clips it as the text grows. 27 fixed `height` declarations on inputs, triggers, filter and pagination chrome, and the bulk-import grid became `min-height` at the same value, matching what `Button` already did. `.clet-launchpad__name` was the worst case, a 16px box holding one clamped line. The six pixel `line-height` values in `UploadField` are now unitless, and the three virtualisers that assumed a 44px row (`Table`, and the bulk-import validate and header-select steps) read the text scale, since a row estimate that drifts from the rendered height overlaps rows.
- **QuickActions**: the item description was `--clet-text-muted` on the tinted item background, which measures 2.03:1 against it, well under the 4.5:1 WCAG AA floor for text that size, and read as barely-there grey. It takes `--clet-text-secondary` now, and steps up to `--clet-text` on hover, where the stronger tint would otherwise eat the gain back. Measured 4.83:1 at rest and 4.97:1 hovered in light, 6.46:1 and 15.04:1 in dark. The hover tint itself is unchanged, so the affordance keeps its strength.
- **UploadField**: the support text names every format in `accept` rather than the first one it recognised. The derivation was an ordered if-chain that returned on its first hit, so `accept=".pdf,.jpg,.jpeg"` announced "Only PDF files are supported" and quietly dropped the images, which is how a field that legitimately takes evidence photos ends up looking like it rejects them. It now reads each entry, so that case says "Only PDF and JPG files are supported". The chain also had no `.jpg`, `.jpeg` or `.png` branch at all, so those fell through to printing the raw attribute back at the user ("Only .jpg,.png are supported"), and with no `accept` it produced "Only any files are supported"; there is now nothing printed when there is nothing to say. `accept` still filters only the OS picker, not a drop, which is now documented rather than implied.
- **TableFilter**: the scroll region is scoped to the popover, so `variant="spread"` no longer clips the shadows on the controls it lays out inline. The two variants share a class for the fields wrapper, and an `overflow` on it turns the header row into a clipping box: on a soft table, where every control sits on `--clet-shadow-soft`, that sliced the shadow off each pill. Setting one axis is enough to cause it, since the other computes from `visible` to `auto`.
- **TableFilter**: the popover caps its height and scrolls instead of running off the screen. A filter with many fields used to grow until the Apply button was somewhere below the fold with no way to reach it. The fields area scrolls between a pinned header and a pinned Apply button, capped by `--clet-table-filter-content-max-height` (400px) and clamped to the space Radix reports as available, so it stays inside the viewport near the bottom of the page as well. `variant="spread"` renders no popover and is unaffected.
- **AppHeaderBranding**: the separator between title and subtitle inherited the document's default size rather than taking one from the token scale, so it stayed put while the two things it divides moved.
- **Dependencies**: cleared all 18 open Dependabot alerts (`pnpm audit` reports zero). `next` 16.3.1 to 16.3.5 (two critical RCEs), `vitest` to 4.1.11, `stylelint` to 17.15.0, and new `pnpm-workspace.yaml` overrides lifting `fast-uri`, `browserslist`, `hono`, `qs` and `js-yaml` to their patched lines. `next` is now an explicit dev dependency so the local copy tracks security fixes instead of floating on the peer range.
- **TableSearch**: the field stretched to match the tallest thing beside it in the header row, so a filter block that wrapped onto three lines turned the search into a tall pill. It is sized by its own content now, and it no longer gives up width to a neighbour.
- **Tabs**: the pill variant's list carried a `margin-bottom` on top of `.clet-tabs`' own flex row gap, so the panel sat ~31px below the tabs instead of the intended 15px. The margin is gone; the container's gap is the single source of that spacing.
- **Sidebar**: the footer no longer draws a top border, and `SidebarHeader`'s title takes the heading face (`--clet-font-header`) by default instead of body copy, so a brand row matches the footer wordmark without the consuming app setting a font.
- **Launchpad**: the trigger painted `--clet-on-primary` (white) and hovered to a hardcoded `#fff3`, on the assumption it always sat on a brand-coloured header. On any light surface the icon was invisible. It now uses `--clet-text-secondary` with the standard hover, matching `AppSwitcher`, and the primary header re-applies the on-primary foreground.
- **SidebarCollapse**: rendered a structural null-vs-button branch straight off the media query, so a static-prerendered page hydrated with a mismatch at mobile widths. It now waits a render past mount, like `SidebarOverlay` and `AppHeader` already did. `SidebarTrigger` carried the mirror image of the same defect and got the same fix.
