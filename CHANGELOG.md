...
- **Rebranded to "Clet"**: "Ghana School of Law" references in prose (package description, README, `getting-started.mdx`) now read "Clet". Docs site header, README title, AGENTS.md title, and the component-authoring cursor rule title also updated. No functional/technical identifiers changed: `--gsl-*` CSS tokens, `.gsl-*` class names, the `@rfdtech/components` package name, and the MCP server id (`gsl-components-docs`) are all unchanged for backward compatibility.
