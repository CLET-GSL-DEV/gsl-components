export const meta = {
  title: "Launchpad",
  description: "Constrained 9-app launcher with a top-right expand button + a See more line under the grid + optional role switcher",
  category: "Components",
  keywords: ["launchpad", "app switcher", "box-grid menu", "app launcher", "product switcher", "grid menu"],
  since: "2.1.0",
  status: "stable",
...
A purpose-built "app directory" launcher for switching between CLET systems — a 3×3 grid of up to 9 apps, with a small ghost expand button at the top right of the panel (always rendered, opens a scaled-up "See all" modal showing every app uncapped), a muted "See more" line under the grid (rendered only when `apps.length` exceeds the 9-app cap, styled as plain text but clickable — opens the same modal as the top-right button), and an optional role switcher below. It grew out of the same underlying popover/grid idea as [`AppSwitcher`](/docs/app-switcher), with a much smaller, more opinionated API and every icon rendered on a fixed brand tile system with no escape hatch.
...
| `children` | `ReactElement<RoleSelectProps>` | — | An optional [`RoleSelect`](/docs/role-select) rendered below the grid, outside the scrollable/masked grid area and separated by a divider — only a `RoleSelect` element is accepted. Omit it and the footer and its divider are not rendered |
| `open` | `boolean` | — | Controlled open state (the popover; the expand modal manages its own state internally) |
| `onOpenChange` | `(open: boolean) => void` | — | Open state callback |
| `trigger` | `ReactNode` | 2x2 box-grid icon | Custom trigger element — still wrapped with the "Open Launchpad" tooltip/accessible name |
