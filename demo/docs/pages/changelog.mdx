...
- `DateRangeSelector` — date range picker with linked From/To inputs, shared calendar, range highlighting
- `SidebarBrand` — compound sub-component for sidebar brand area, auto-hides on collapse
- `useTableFilter` hook — URL search-param based filter state
...
- AppHeaderProfile popover uses `--gsl-primary` bg + `--gsl-on-primary` text for avatar
- AppHeader uses full width and falls back to `1rem` for `--gsl-app-layout-body-gap`
...
- `AppSidebar` renders as `<aside>` with ref forwarding
- CommandGroup loading replaced with 3-row skeleton (was thin loading bar)
- SidebarContent scrollbar hidden; scroll hint and gradients internalized
