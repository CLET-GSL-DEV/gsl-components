...
### Breaking
- **ValidateDataStep**: Replaced `<Table />` with a custom virtualized table built on `@tanstack/react-virtual`. Removed all `<Table />`, `TableBulkActions`, and `TableFooter` usage. Error cells now use background color only (no border). Added `onResetDiscardedRows` prop and a "Reset discarded rows" button in the header. Error counts now correctly exclude discarded rows.
...
- BulkImportModal: `resetDiscardedRows` method on `UseBulkImportFlowReturn`
- BulkImportModal: "Reset discarded rows" button in ValidateDataStep header

### Changed
- BulkImportModal: ValidateDataStep now computes `activeErrors` filtered by `discardedRows` for accurate error counts
