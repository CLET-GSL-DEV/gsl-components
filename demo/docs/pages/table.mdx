...
Compound table component with built-in URL-driven search, pagination, filters, column sorting, selection, and loading skeletons. All state lives in the URL — components read and write search params internally. Pass `paramPrefix` to `<Table>` to namespace state for multiple tables on one page.
...
When `selectable` is true, a checkbox column is prepended to the table. The header checkbox toggles all visible rows (shows indeterminate state when partially selected). Individual row checkboxes toggle that row by its `rowKey` value. Selection is fully controlled: provide `selectedIds` and `onSelectionChange` to manage state externally. Sample usage:
...
| Token | Effect |
|-------|--------|
| `--gsl-rounded-base` | All border radii |
