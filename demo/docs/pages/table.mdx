...
| `defaultSelectedIds` | `Set<string \| number>` | `new Set()` | Initial set of selected row keys |
...
When `selectable` is true, a checkbox column is always present, collapsed to 0 width when no rows are selected. The header checkbox toggles all visible rows (shows indeterminate state when partially selected).
...
Pass `defaultSelectedIds` to pre-select rows on first render, then use `onSelectionChange` to track changes for `TableBulkActions`.
