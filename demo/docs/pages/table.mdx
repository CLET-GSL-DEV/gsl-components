...
| `selectable` | `boolean` | `false` | Show a checkbox selection column with select-all in the header. Column is hidden until at least one row is selected. |
...
| `rowActions` | `TableRowAction[]` | — | Custom actions shown in the right-click popover. "Select" always appears first when `selectable` is true. |
| `bulkActions` | `TableBulkAction[]` | — | Actions shown in the right-click popover when multiple rows are selected. Each action receives the full `selectedIds` set. |
| `onRowClick` | `(rowKey: string \| number) => void` | — | Fired on left-click of a data row |
| `onRowContextMenu` | `(rowKey: string \| number, event: React.MouseEvent) => void` | — | Overrides the built-in right-click popover with a custom handler |
...
When `selectable` is true, a checkbox column appears only after at least one row is selected (via right-click → "Select"). The header checkbox toggles all visible rows (shows indeterminate state when partially selected).

Rows show hover state and cursor pointer when `selectable`, `onRowClick`, `rowActions`, `bulkActions`, or `onRowContextMenu` is set.

### Right-click context menu

Right-clicking a data row opens a popover with:
- **Select** / **Deselect** — toggles the clicked row (only when `selectable` is true)
- **Bulk actions** — from `bulkActions` prop, shown when multiple rows are selected (each action operates on all `selectedIds`)
- **Row actions** — from `rowActions` prop, operates on the clicked row

Use `onRowContextMenu` to replace the built-in popover with your own handler entirely.
...
| `onClick` | `(rowKey: string \| number) => void` | Called with the row key of the right-clicked row |
