...
<Table paramPrefix="members">
  ...
  <TableFilter>                → writes ?members.f_role=admin
    ...
    <Dropdown value={roleValue} onValueChange={setRoleValue} ... />
  </TableFilter>
...
Filter fields must be *named* form-associated elements for `FormData` to capture them — never a raw `<select>`/native `<button>`/native `<input>` as the visible control. Pair a hidden `<input type="hidden" name="...">` (captured by `FormData`) with a real `Dropdown` (or other library input) for the visible, styled UI.
...
Filter fields must be *named* form-associated elements for `FormData` to read them — never a raw `<select>`/native `<button>`/native `<input>` as the visible control. Pair a hidden `<input type="hidden" name="status" value={statusValue} />` (captured by `FormData`) with a `Dropdown` (or other library input) driving that value for the visible, styled UI. The `name` becomes the URL key.
