...
| `filteredEmptyText` | `ReactNode` | `"No matching results"` | Title shown instead of `emptyText` when a search term or filter values are active |
| `filteredEmptyDescription` | `ReactNode` | `"Try removing a filter..."` | Supporting copy for the filtered-empty state |
| `emptyContent` | `ReactNode` | — | Rich empty state rendered in place of the icon + text when `data` is empty (e.g. `<EmptyState illustration={...} title={...} />`). Headers still render above it. |
