...
import { SidebarBrandPreview } from "../previews/code/SidebarBrandPreview";
...
import sidebarBrandSource from "../previews/examples/sidebar-brand.example.tsx?raw";
...
- **`"primary"`**: the 2.3 brand rail: `--clet-primary-surface` background, `--clet-on-primary` text, no radius and no border. Frozen at the 2.3 look — prefer `"brand"` for new work. Branding goes in a `SidebarHeader` at the top of the rail, and the group labels set in uppercase with the fold chevron pointing up while a group is open.
- **`"brand"`** (2.4): the navy rail — `#051b2c` surface with a single artwork image docked right and vertically centered at 30% opacity, image-only header (no title text), 12px uppercase `#76808d` group labels, 36px items, and a white active item with navy text. The rail of the 2.4 layout shell, paired with `AppHeader variant="plain"`.

<ExampleTabs
  title='variant="brand": the 2.4 navy rail with mosaic image and image-only header'
  preview={<SidebarBrandPreview />}
  code={sidebarBrandSource}
/>
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `variant` | `"default" \| "plain" \| "primary" \| "brand"` | `"default"` | Visual style: see [Variants](#variants) |
| `mobileHeader` | `ReactNode` | — | Content rendered into a mobile-only `SidebarHeader` (hidden on desktop via the built-in `clet-sidebar__header--mobile-only` class) when `variant` is `"plain"`, `"primary"`, or `"brand"`. Typically a `SidebarBrand` with the same logo/title as the sibling `AppHeader`'s `AppHeaderBranding`, so the brand is visible when the mobile drawer opens. Not auto-extracted by `AppLayout`: pass it explicitly. Skipped if you also supply your own `<SidebarHeader>` child, which is the usual composition on the `"primary"` and `"brand"` rails. |
...
Props for `Sidebar`. Includes `variant?: "default" | "plain" | "primary" | "brand"` and `mobileHeader?: ReactNode`, see [Variants](#variants) and the `Sidebar` prop table above.
