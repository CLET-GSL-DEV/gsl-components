...
Part-level class names merged after base `gsl-combobox*` styles:
...
|-------|---------|----------|
| `--gsl-combobox-trigger-height` | `40px` | Trigger button height |
| `--gsl-combobox-radius` | `var(--gsl-radius-base)` | Trigger and popover corner radius |
| `--gsl-combobox-font` | `var(--gsl-font-body)` | Font family |
| `--gsl-combobox-label-size` | `var(--gsl-text-size-base)` | Trigger/search/item font size |
| `--gsl-combobox-menu-max-height` | `320px` | Max height of the scrollable option list |
...
- The multi-select checkbox visual reuses the same checked-state colors as `Checkbox` (`--gsl-primary` background, white check); single-select shows a plain check mark instead of a checkbox box.
