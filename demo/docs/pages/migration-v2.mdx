export const meta = {
  ...
  category: "Guide",
  keywords: ["migration", "upgrade", "v2", "breaking changes", "deprecated", "new design system", "AppHeaderProfile", "ProfilePopover", "Launchpad", "AppSwitcher", "rebrand", "AppLayout", "Sidebar", "RoleSelect", "stacked layout", "CountrySelector", "Combobox"],
  since: "2.0.0",
  status: "guide",
  related: ["changelog", "getting-started", "theme", "app-header", "app-switcher", "launchpad", "table", "sidebar", "profile-popover", "role-select", "app-layout", "metric-card", "tabs", "country-selector", "combobox"],
};
...
**To override:** put the call in `src/gsl.theme.ts` (see [Theme](/docs/theme#recommended-file-srcgsltheme-ts) for the recommended file convention) and side-effect-import it once in your entry point:
...
**Migrating a project that already has `--gsl-*` token overrides?** Find every existing override of
a *known* `--gsl-*` token — a `gslTheme()` call (`src/gsl.theme.ts` or elsewhere) or hand-written
CSS setting a real `--gsl-*` custom property, cross-checked against `get_tokens()`. This is about
...
- [ ] If the project already has `--gsl-*` token overrides, list every known-token override found and ask the user per-token whether to keep it or adopt the new default — never decide silently (§5)
