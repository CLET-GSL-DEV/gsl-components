...
import { SKILL_SOURCE_PATH } from "../paths.js";
...
export async function runSetup(cwd: string = process.cwd()): Promise<void> {
  ...
  // Always drop the cross-tool skill, regardless of which specific tool (if any) is detected.
  const aiDir = path.join(cwd, ".ai");
  await mkdir(aiDir, { recursive: true });
  await copyFile(SKILL_SOURCE_PATH, path.join(aiDir, "SKILL.md"));
  console.log("✓ .ai/SKILL.md written\n");

  const results: InstallResult[] = [];
  if (claude.detect(cwd)) results.push(await claude.install(cwd, SKILL_SOURCE_PATH));
  if (cursor.detect(cwd)) results.push(await cursor.install(cwd, SKILL_SOURCE_PATH));
