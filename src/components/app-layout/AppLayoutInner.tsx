...
export const AppLayoutInner = forwardRef<HTMLDivElement, AppLayoutInnerProps>(
  function AppLayoutInner({ children, className, variant = "default" }, ref) {
    ...
    const sidebarNode = sidebarEl && (
      <div
        className={cn("clet-app-layout__sidebar", sidebar.className)}
        {...sidebar.rest}
      >
        {sidebar.children}
      </div>
    ...
    const breadcrumbNode = items.length > 0 && (
      <div className="clet-app-layout__breadcrumb">
        <Breadcrumb>
          <BreadcrumbList>
            {items.flatMap((item, i) => [
              ...
              <BreadcrumbItem key={i}>
                {item.href ? (
                  <Link to={item.href} className="clet-breadcrumb__link">
                    {item.label}
                  </Link>
                ) : (
    ...
    const contentNode = bodyEl && (
      <div
        className={cn("clet-app-layout__content", body.className)}
        {...body.rest}
      >
        {body.children}
      </div>
    ...
    if (variant === "stacked") {
      return (
        <div
          ref={ref}
          className={cn("clet-app-layout", "clet-app-layout--stacked", className)}
        >
          {headerEl}
          <div className="clet-app-layout__row">
            {sidebarNode}
            <div className="clet-app-layout__main">
              {breadcrumbNode}
              {contentNode}
            </div>
          </div>
    ...
    return (
      <div ref={ref} className={cn("clet-app-layout", className)}>
        {sidebarNode}
        <div className="clet-app-layout__body">
          {headerEl}
          {breadcrumbNode}
          {contentNode}
        </div>
