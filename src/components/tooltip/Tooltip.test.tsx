...
describe("Tooltip", () => {
	...
	it("keeps focus on the trigger while open", async () => {
		...
		target.focus();
		await waitFor(() => {
			expect(document.querySelector('[role="tooltip"]')).toHaveClass(
				"clet-tooltip__content--open",
			);
		});
		expect(document.activeElement).toBe(target);
	});

	it("stays closed after blur without refocusing the trigger", async () => {
		render(
			<Tooltip content="Helper text">
				<button type="button">Hover me</button>
			</Tooltip>,
	...
	it("renders ReactNode content", async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByTestId("rich-content")).toHaveTextContent(
				"rich content",
			);
		});
	});
