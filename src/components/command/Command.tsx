    ...
    return (
      <PopoverPrimitive.Root open={open} onOpenChange={setOpen} modal={false}>
        ...
        >
          <CommandPrimitive
            ref={ref}
            className={cn("clet-command gsl-command", classNames?.root, className)}
            {...props}
          >
            {children}
          </CommandPrimitive>
  ...
  function CommandDialog(
    ...
    return (
      <CommandPrimitive.Dialog
        ref={ref}
        className={cn(
          "clet-command gsl-command",
          "clet-command--dialog gsl-command--dialog",
          classNames?.dialog,
          className,
        )}
        overlayClassName={cn(
          "clet-command-dialog__overlay gsl-command-dialog__overlay",
          classNames?.overlay,
          overlayClassName,
        )}
        contentClassName={cn(
          "clet-command-dialog__content gsl-command-dialog__content",
          classNames?.content,
          contentClassName,
        )}
        {...dialogOpenProps}
...
export const CommandInput = forwardRef<HTMLInputElement, CommandInputProps>(
  function CommandInput({ classNames, className, shortcut, ...props }, ref) {
    ...
    const input = (
      <>
        <Search
          className="clet-command__input-icon gsl-command__input-icon"
          aria-hidden="true"
          size={16}
          strokeWidth={2}
        />
        <CommandPrimitive.Input
          ...
          className={cn("clet-command__input gsl-command__input", classNames?.input, className)}
          onInput={(e) => setHasValue((e.target as HTMLInputElement).value.length > 0)}
          {...props}
        />
        {hasValue && (
          <div
            className="clet-command__input-clear gsl-command__input-clear"
            onClick={() => {
              if (inputRef.current) {
        ...
        {!hasValue && shortcutLabels.length > 0 ? (
          <kbd className="clet-command__input-shortcut gsl-command__input-shortcut" aria-hidden="true">
            {shortcutLabels.map((label, index) => (
              <span
                key={`${label}-${index}`}
                className="clet-command__shortcut-key gsl-command__shortcut-key"
              >
                {label}
              </span>
            ))}
      ...
      return (
        <PopoverPrimitive.Anchor asChild>
          <div
            ...
            className={cn(
              "clet-command__input-wrapper gsl-command__input-wrapper",
              classNames?.wrapper,
            )}
            onFocus={handleFocus}
            onBlur={handleBlur}
    ...
    return (
      <div
        className={cn("clet-command__input-wrapper gsl-command__input-wrapper", classNames?.wrapper)}
      >
        {input}
      </div>
    );
...
export const CommandList = forwardRef<HTMLDivElement, CommandListProps>(
  function CommandList({ classNames, className, children, ...props }, ref) {
    ...
    return (
      <CommandPrimitive.List
        ref={ref}
        className={cn("clet-command__list gsl-command__list", classNames?.list, className)}
        {...props}
      >
        {children}
      </CommandPrimitive.List>
  ...
  function CommandEmpty({ classNames, className, children, ...props }, ref) {
    return (
      <CommandPrimitive.Empty
        ref={ref}
        className={cn("clet-command__empty gsl-command__empty", classNames?.empty, className)}
        {...props}
      >
        {children}
      </CommandPrimitive.Empty>
  ...
  function CommandGroup(
    ...
    return (
      <CommandPrimitive.Group
        ref={ref}
        className={cn("clet-command__group gsl-command__group", classNames?.group, className)}
        {...props}
      >
        {loading ? (
          <div
            className={cn(
              "clet-command__group-loading gsl-command__group-loading",
          ...
          >
            {[1, 2, 3].map((i) => (
              <div key={i} className="clet-command__skeleton gsl-command__skeleton">
                <div className="clet-command__skeleton-line gsl-command__skeleton-line" />
              </div>
            ))}
          </div>
        ) : (
  ...
  function CommandItem({ classNames, className, children, ...props }, ref) {
    return (
      <CommandPrimitive.Item
        ref={ref}
        className={cn("clet-command__item gsl-command__item", classNames?.item, className)}
        {...props}
      >
        {children}
      </CommandPrimitive.Item>
...
export const CommandShortcut = forwardRef<HTMLElement, CommandShortcutProps>(
  function CommandShortcut({ classNames, className, children }, ref) {
    return (
      <kbd
        ref={ref}
        className={cn("clet-command__shortcut gsl-command__shortcut", classNames?.shortcut, className)}
      >
        {children}
      </kbd>
    );
...
>(function CommandSeparator({ classNames, className, ...props }, ref) {
  return (
    <CommandPrimitive.Separator
      ref={ref}
      className={cn("clet-command__separator gsl-command__separator", classNames?.separator, className)}
      {...props}
    />
  );
});
  ...
  function CommandLoading({ classNames, className, children, ...props }, ref) {
    return (
      <CommandPrimitive.Loading
        ref={ref}
        className={cn("clet-command__loading gsl-command__loading", classNames?.loading, className)}
        {...props}
      >
        {children}
      </CommandPrimitive.Loading>
