...
export function AppIconTile({ name, children, className }: AppIconTileProps) {
  ...
  return (
    <span
      className={cn("clet-app-switcher__tile", className)}
      style={{ background }}
    >
      <svg className="clet-app-switcher__tile-pattern" aria-hidden="true" focusable="false">
        <TilePattern index={patternIndex} patternId={patternId} />
      </svg>
      <span className="clet-app-switcher__tile-glyph">{children}</span>
    </span>
  );
}
